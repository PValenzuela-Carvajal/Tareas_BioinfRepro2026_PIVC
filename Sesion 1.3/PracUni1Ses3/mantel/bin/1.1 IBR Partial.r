...
##Partial Mantel TEST 
##Flat matrix as distance matrix 
B.flat.dist<-as.dist(B.flat)

##Save results

PartialIBRresults <- c("scenario", "partial_r", "pvalue", "nperm")
## For loop partial mantel
# Scenarios to evaluate: present and the two LGM models
for(i in c("present", "ccsm", "miroc")) {
  
  print(paste("Partial Mantel results for", i))
  
  # Effective distance matrix for scenario i
  B.scen.dist <- as.dist(get(paste0("B.", i)))
  
  # Linearized Fst as a distance matrix
  B.FstLin.dist <- as.dist(B.FstLin)
  
  # Partial Mantel test:
  #   correlation between B.scen.dist and B.FstLin.dist
  #   while controlling (partialling out) the effect of B.flat.dist
  pm <- mantel.partial(
    xdis = B.scen.dist,      # matrix 1 (effective distance of the scenario)
    ydis = B.FstLin.dist,    # matrix 2 (linearized Fst)
    zdis = B.flat.dist,      # control matrix (partialled out)
    method = "pearson",
    permutations = 10000
  )
  ...
  # Plot: Fst residuals after removing the effect of flat
  # vs. effective distance residuals after removing the effect of flat
  DistPlot(get(paste0("B.", i)), B.FstLin, plotnames = FALSE,
           ylabel = expression("F"[ST]*"/(1 - "[FST]*")"),
           xlabel = paste("Effective distance", i,  "| flat"))
  
  # Save results
  PartialIBRresults <- rbind(
    PartialIBRresults,
    c(i, round(pm$statistic, 4), round(pm$signif, 6), 10000)
  )
}
